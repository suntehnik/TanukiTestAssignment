 ...
 * presented side-by-side with a list of items in a {@link The500PxActivity}
 * .
 * <p>
 * This activity is mostly just a 'shell' activity containing nothing more than
 * a {@link CategoryImageListFragment}.
 */
public class CategoryImageListActivity extends FragmentActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (savedInstanceState == null) {
			...
			int categoryId = getIntent()
					.getIntExtra(CategoryImageListFragment.ARG_ITEM_ID, -1);
			arguments.putInt(CategoryImageListFragment.ARG_ITEM_ID, categoryId);
			setTitle(CategoryContent.categories.get(categoryId));
			CategoryImageListFragment fragment = new CategoryImageListFragment();
			fragment.setArguments(arguments);
			getSupportFragmentManager().beginTransaction()
					.add(R.id.webimage_detail_container, fragment).commit();
		}
	}
